...
Deno.test("serverConfigSchema should validate http server with OAuth config (clientId only)", () => {
  const valid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
  ...
  if (result.success && result.data.type === "http") {
    assertEquals(result.data.oauth?.clientId, "test-client-id");
  }
});

Deno.test("serverConfigSchema should validate http server with OAuth config including clientSecret", () => {
  const valid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
  ...
  if (result.success && result.data.type === "http") {
    assertEquals(result.data.oauth?.clientId, "test-client-id");
    assertEquals(result.data.oauth?.clientSecret, "test-secret");
  }
});

Deno.test("serverConfigSchema should validate http server with OAuth config including scopes", () => {
  const valid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
  ...
  assertEquals(result.success, true);
  if (result.success && result.data.type === "http") {
    assertEquals(result.data.oauth?.scopes, ["read", "write"]);
  }
});

Deno.test("serverConfigSchema should validate sse server with OAuth config", () => {
  const valid = {
    type: "sse",
...
Deno.test("serverConfigSchema should reject OAuth config with empty clientId", () => {
  const invalid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
...
Deno.test("serverConfigSchema should reject OAuth config with manual authorizationUrl", () => {
  const invalid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
  ...
  if (!result.success) {
    assertEquals(
      result.error.errors[0].message.includes("Manual OAuth endpoints"),
      true,
    );
  }
});

Deno.test("serverConfigSchema should reject OAuth config with manual tokenUrl", () => {
  const invalid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
...
Deno.test("serverConfigSchema should reject OAuth config with manual redirectUri", () => {
  const invalid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
...
Deno.test("serverConfigSchema should reject OAuth config with manual flow field", () => {
  const invalid = {
    type: "http",
    url: "https://api.example.com/mcp",
    oauth: {
...
Deno.test("toolscriptConfigSchema should validate config with OAuth-protected servers", () => {
  const config = {
    mcpServers: {
      github: {
        type: "http",
  ...
  assertEquals(result.success, true);
  if (result.success) {
    assertEquals(Object.keys(result.data.mcpServers).length, 2);
  }
});
